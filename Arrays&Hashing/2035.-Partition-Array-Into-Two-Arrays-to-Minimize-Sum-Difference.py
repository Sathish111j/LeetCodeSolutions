class Solution:
    def minimumDifference(self, nums: List[int]) -> int:
        ...
        total = sum(nums) 
        half = total // 2 # the best sum required for each, have to find sum nearest to this
        for k in range(1, N):
            left = left_sums[k] # if taking k no. from left_sums
            right = right_sums[N-k] # then have to take remaining N-k from right_sums.
            right.sort() # sorting, so that can binary search the required value
            for x in left:
                r = half - x # required, how much need to add in x to bring it closer to half.
                p = bisect.bisect_left(right, r) # finding index of value closest to r, present in right, using binary search
                for q in [p, p-1]:
                    if 0 <= q < len(right):
